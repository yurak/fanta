RSpec.describe TelegramBot::Auction::SquadCompleteNotifier do
  describe '#call' do
    ...
    let(:notification) { create(:notification, notifiable: auction_round, team: team, kind: :auction_squad_complete) }
    ...
    context 'when team has a user' do
      ...
      it 'uses squad_complete translation key' do
        service_call

        expect(I18n).to have_received(:t).with('telegram.notifier.auction.squad_complete', hash_including(locale: :ua))
      end
    end
  end
end
