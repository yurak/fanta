...
RSpec.describe TelegramBot::Tour::OpenedNotifier do
  def build_notifiable(league: nil)
    instance_double(Tour, league: league, number: 1, tournament_round: instance_double(TournamentRound, deadline: Time.current))
  end
  ...
  describe '#call' do
    ...
    it 'calls Sender service and returns true' do
      notification = build_valid_notification
      stub_dependencies(notification.notifiable)
